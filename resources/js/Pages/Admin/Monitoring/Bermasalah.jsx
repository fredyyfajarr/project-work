...
export default function Bermasalah({ data = {} }) {
    ...
    return (
        <AdminLayout title="Mahasiswa Bermasalah Akademik">
            <PageHeader
                title="Mahasiswa Bermasalah Akademik"
                subtitle="Monitoring mahasiswa dengan IPK di bawah 2.75."
                actions={[{ label: 'Kembali', href: '/admin/monitoring', variant: 'secondary' }]}
            />
            <div className="rounded-2xl bg-white p-4 shadow-sm">
                <div className="overflow-x-auto rounded-2xl border border-slate-100">
                    <table className="w-full min-w-[760px] align-middle text-sm">
                        <thead>
                        ...
                        <tbody>
                            {rows.map((m, i) => {
                                ...
                                return (
                                    <tr key={m?.idMahasiswa || m?.id || i} className="border-t border-slate-100 hover:bg-blue-50/40">
                                        ...
                                        <td className="px-4 py-3 font-semibold text-red-600">{ak?.ipk ?? '-'}</td>
                                        <td className="px-4 py-3">
                                            {!Number.isNaN(ipk) && ipk < 2.0 ? (
                                                <span className="rounded-full bg-red-100 px-3 py-1 text-xs font-bold text-red-700">Kritis</span>
                                            ) : (
                                                <span className="rounded-full bg-yellow-100 px-3 py-1 text-xs font-bold text-yellow-700">Perlu Pendampingan</span>
                                            )}
                                        </td>
                                        <td className="px-4 py-3">
                            ...
                            {rows.length === 0 && (
                                <tr><td colSpan="7" className="px-4 py-6 text-center text-slate-400">Tidak ada mahasiswa bermasalah akademik.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
